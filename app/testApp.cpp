...
const char* const testApp::LAUNCHABLE_NAME = "GattData";
...
bool testApp::initModule()
{
    mModuleState = WB_RES::ModuleStateValues::INITIALIZED;
    return true;
}

void testApp::deinitModule()
{
    mModuleState = WB_RES::ModuleStateValues::UNINITIALIZED;
}

bool testApp::startModule()
{
...
testApp::DataSub* testApp::findDataSub(const wb::ResourceId resourceId)
{
    for (size_t i=0; i<MAX_DATASUB_COUNT; i++)
    {
...
testApp::DataSub*  testApp::findDataSubByRef(const uint8_t clientReference)
{
    for (size_t i=0; i<MAX_DATASUB_COUNT; i++)
    {
        const DataSub &ds= mDataSubs[i];
...
testApp::DataSub* testApp::getFreeDataSubSlot()
{
    for (size_t i=0; i<MAX_DATASUB_COUNT; i++)
    {
        const DataSub &ds= mDataSubs[i];
...
void testApp::sendPacket(const uint8_t data[], size_t len, uint8_t reference /*=0*/, uint8_t type /*=2*/){
    len = len<64? len:64; 
    mDataMsgBuffer[0]=type;
    mDataMsgBuffer[1]=reference;
...
void testApp::handleIncomingCommand(const wb::Array<uint8> &commandData)
{
    ...
    handleCommand(cmd, reference, pData, dataLen);
}

bool testApp::subscribe(const char path[], size_t pathlen, uint8_t reference){
...
void testApp::unsubscribe(uint8_t reference){
...
void testApp::ledSet(bool on){
    wb::ResourceId led;
    getResource("Component/Led",led);
    asyncPut(led, AsyncRequestOptions::Empty, on);
}

void testApp::ledSetPattern_n(uint16_t* pattern, size_t length, bool startOn /*=true*/){
    stopTimer(mLedTimer);
...
void testApp::ledSetPattern(uint16_t* pattern, bool startOn /*=true*/){
    ledSetPattern_n(pattern, -1, startOn);
}

void testApp::onGetResult(wb::RequestId requestId,
                                      wb::ResourceId resourceId,
                                      wb::Result resultCode,
                                      const wb::Value& rResultData)
{
    DEBUGLOG("testApp::onGetResult");
...
void  testApp::onSubscribeResult(wb::RequestId requestId,
                                              wb::ResourceId resourceId,
                                              wb::Result resultCode,
                                              const wb::Value& rResultData)
{
    ...
    switch (resourceId.localResourceId)
    {
        ...
        default:
        {
            ...
            testApp::DataSub *ds = findDataSub(resourceId);
            if (ds == nullptr)
            {
...
void testApp::unsubscribeAllStreams()
{
    for (size_t i=0;i<MAX_DATASUB_COUNT; i++)
    {
...
void testApp::onNotify(wb::ResourceId resourceId,
...
void testApp::onPostResult(wb::RequestId requestId, 
                                       wb::ResourceId resourceId, 
                                       wb::Result resultCode, 
                                       const wb::Value& rResultData)
{
    DEBUGLOG("testApp::onPostResult: %d", resultCode);
